...
class AdminTopic extends Form
{
	...
	function adminTopic()
	{
		...
		foreach($list_data as $row)
		{
			...
			if($row['property'] > 0){
				if($row['property']==1)
					joc()->set_var('set_property','<a href="javascript:;" onclick="updateProperty('.$row['id'].',0)">Hủy bỏ hiển thị</a><br/><a href="javascript:;" onclick="updateProperty('.$row['id'].',4)">Thiết lập trang chủ</a>');
				if($row['property']==4)
				...
				joc()->set_var('set_property','<a href="javascript:;" onclick="updateProperty('.$row['id'].',1)">Thiết lập hiển thị</a><br/><a href="javascript:;" onclick="updateProperty('.$row['id'].',4)">Thiết lập trang chủ</a>');
			}	

...
$list_news_all=$newsObj->getNews('store','id,title,description,time_public,cate_id,time_created,img1,img2',' ( cate_path LIKE "%,273,%" OR cate_path LIKE "%,288,%" OR cate_path LIKE "%,283,%" ) AND time_public > 0 AND time_public < '.time(),'time_public DESC',6,'id',true);
$arr_news=array();
foreach($list_news_all as $row)
{
	...
	$arr_news[$row['id']]['img']=IMG::show($newsObj->getPathNews($row['time_created']),$row['img1'],'3');

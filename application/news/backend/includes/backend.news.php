...
Class BackendNews
{
    ...
    public function getReviewOne($news_id)
    {
        ...
        return self::$dbNews->selectOne('review', 'id,poll_id,is_video,relate,user_id,cate_id,cate_path,cate_other,is_img,title,description,content,img1,img2,img3,img4,province_id,district_id,status,type,origin,author,tag,file,arrange,censor_id,reason_return,time_public,time_created,editor_id,type_post,topic_id', 'id=' . $news_id);
    }
    ...
    public function delReview($news_id)
    {
        ...
        $data = array('id' => $news_id, 'user_id' => $user_info['id'], 'title' => json_encode($review), 'time_delete' => date('Y-m-d H:i:s'));
    ...
    public function getStoreOne($news_id)
    {
        settype($news_id, 'int');
        if (!$news_id) return array();
        return self::$dbNews->selectOne('store', 'id,user_id,cate_id,cate_path,cate_other,title,description,img1,img2,img3,img4,province_id,district_id,status,type,origin,author,tag,file,arrange,censor_id,time_public,time_created,is_video,is_img,relate,editor_id,type_post,topic_id,poll_id', 'id=' . $news_id);
    }
    ...
    public function insertReviewToHome($news_id, $censor_id, $time_public, $type_img_show = 1, $property = 0)
    {
        ...
        $id_news_hit = self::$dbNews->insert('store_hit', array('nw_id' => $id_news_store, 'hit' => 0, 'user_id' => $row['user_id'], 'time_created' => $row['time_created'], 'cate_path' => $row['cate_path'], 'type_post' => $row['type_post']));
        $id_news_content = self::$dbNews->insert('store_content', array('nw_id' => $id_news_store, 'content' => $row['content']));
        //if($id_news_content)
        self::$dbNews->delete('review', 'id=' . $news_id, 1);
        self::$review_id = $id_news_store;
        return array('store_id' => $id_news_store, 'home_id' => $id_news_home, 'view_id' => 1, 'search_id' => $id_news_search, 'hit_id' => $id_news_hit, 'content_id' => $id_news_content);
    }
    ...
    public function insertReviewToStore($news_id, $censor_id, $time_public, $property = 0)
    {
        ...
        $arrNewsSearch['keyword'] = str_replace(array('"', '”', "'", '“'), array('', '', '', ''), $arrNewsSearch['keyword']);
        $id_news_search = self::$dbNews->insert('search', $arrNewsSearch);
        $id_news_hit = self::$dbNews->insert('store_hit', array('nw_id' => $id_news_store, 'hit' => 1, 'time_created' => $row['time_created'], 'cate_path' => $row['cate_path']));
        $id_news_content = self::$dbNews->insert('store_content', array('nw_id' => $id_news_store, 'content' => $row['content']));
        //if($id_news_content)
        self::$dbNews->delete('review', 'id=' . $news_id, 1);
        return array('store_id' => $id_news_store, 'search_id' => $id_news_search, 'hit_id' => $id_news_hit, 'content_id' => $id_news_content);
    }
    ...
    public function getMultiPathNews($cate_ids = '', $news_ids = '')
    {
        ...
        $path = array();
        if ($cate_ids) {
            $cates = self::$dbNews->select('category', 'id,name,cate_id1,cate_id2,cate_id3,cate_id4,cate_name1,cate_name2,cate_name3,cate_name4', 'property & 1 = 1');
    ...
    {
        return self::$dbNews->select('region_category', 'id,cate_id,region_id,skins_type,number_record,arrange,property', $wh, $order, $limit, $key);
    }
    ...
    public function mapNewsRegion($arr_news, $arr_region)
    {
        ...
        if (!(count($arr_news) && count($arr_region))) return false;
    ...
    public function deleteStore($id)
    {
        ...
        $user_info = UserCurrent::$current->data;
        $data = array('id' => $id, 'user_id' => $user_info['id'], 'title' => json_encode($detail), 'time_delete' => date('Y-m-d H:i:s'));
        $this->insertData('log_delete_from_store', $data);
    ...
    public function getNews($table, $list_field, $wh = null, $order = null, $limit = null, $key = null, $is_cache = false)
    {
    ...
    public function searchFullTextNews($keyword, $wh = '', $limit = '0,20')
    {
        if ($keyword) {
            ...
            if (count($data)) {
                ...
                $result = $this->getNews('store', 'id,title,cate_id,img2,description,time_created,time_public', "id IN ({$news_ids})", 'time_public DESC', null, 'id');
                foreach ($data as $_temp) {
    ...
    public function log($action, $id, $ext = '')
    {
        $user_info = UserCurrent::$current->data;
        if ($ext == '')
            $data = $user_info['user_name'] . ' - ' . $action . '. Bài có id: ' . $id . ' lúc: ' . date('H:i:s d/n/Y', time()) . '-' . $_SERVER['HTTP_X_REAL_IP'] . '<br/>';
        else
            $data = $user_info['user_name'] . ' - ' . $action . '. Bài có id: ' . $id . ' (' . $ext . ') lúc: ' . date('H:i:s d/n/Y', time()) . '-' . $_SERVER['HTTP_X_REAL_IP'] . '<br/>';
